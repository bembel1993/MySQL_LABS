...
SELECT COUNT(*) FROM [DBO].[OFFICES] WHERE [OFFICE] BETWEEN 11 AND 13 AND [MGR]=104; --КОЛИЧЕСТВО СТОЛБЦОВ 1
select * from offices
...
SELECT * FROM [DBO].[CUSTOMERS]; --ИНДЕКС ПОКРЫТИЯ - это индекс , включающий все столбцы, полученные по запросу.
...
SELECT DISTINCT
	ID_Преп, Ном_Предмета, ID_зан 
FROM Пов_кв
WHERE Ном_Предмета != ID_зан
